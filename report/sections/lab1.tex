...
Это требование нетривиально: если $h$ слишком велико, стенка
покрывается одним-двумя элементами и численный метод не разрешает
температурного (или механического) поля внутри неё. Минимальное число
элементов на толщину~--- это по сути дискретный аналог \textit{условия
Найквиста}: нужно не менее двух-трёх точек на «полуволну» поля.
...
Вид сбоку показывает, что в толщину стенки укладывается 3--4 элемента, как и планировалось. Внутри полости элементы вытянутые, но это неизбежно при таком соотношении размеров: радиус $R$ в 4 раза больше толщины $\Delta r$, и при $h = 0.2$ по окружности укладывается примерно 20 элементов, а по толщине~--- 3--4.
...
без аналитического описания. Но стоит заметить, что иногда может быть очень трудно разбить сложную STL-модель на патчи, пригодные для генерации качественной сетки. В таких случаях нужно как-то в ручную тратить модель.