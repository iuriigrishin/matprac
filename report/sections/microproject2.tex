...
константы берутся из таблицы материалов, приведенной в том же источнике, что и модель.
...
\begin{equation}
  ...
  z_s=z_{\min},
\end{equation}
а ширина берётся как $\sigma=0{,}1\,D$, где $D$~--- максимальный размер
bounding box модели.

Временная форма~--- оконный ультразвуковой burst:
...
\]
Множитель $\sin^2(\pi\tau/T)$ плавно поднимает амплитуду от нуля и плавно возвращает её в ноль.
Это уменьшает артефакты, которые появились бы при резком обрыве синуса.
...
\texttt{assemble\_matrix}. Матрица масс используется в lumped-виде:
вместо полной матрицы хранится диагональный вектор сумм по строкам.
Это делает явный шаг дешёвым: не нужно решать линейную систему на
...
Примеры видео вынесены на
\href{https://disk.360.yandex.ru/d/l9_8UlurSN1kMA}{Яндекс Диск}.
