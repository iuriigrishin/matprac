...
double Solver::stable_dt(const Grid& grid, double safety) {
  ...
  double chi   = std::max(chi_s, chi_l);  // худший случай — большая диффузия
  double h_min = std::min({grid.dx(), grid.dy(), grid.dz()});
  return safety * h_min * h_min / (6.0 * chi);  // 0.5 * dx^2 / (3*chi), d=3
}

static inline double k_face(double k1, double k2) {
  if (k1 + k2 <= 0.0) return 0.0;
...
void Solver::step(Grid& grid, double dt) {
  ...
  // dH/dt = - (F_{i+1/2} - F_{i-1/2}) / dx  + аналог по y, z
  // F через грань = -k_face * (T_{i+1} - T_i) / dx
  for (int k = 1; k < nz - 1; ++k) {
    for (int j = 1; j < ny - 1; ++j) {
      for (int i = 1; i < nx - 1; ++i) {
        ...
        double Tx_p = grid.T(i + 1, j, k); double kx_p = k_face(k0, grid.conductivity(i + 1, j, k));
        double Tx_m = grid.T(i - 1, j, k); double kx_m = k_face(k0, grid.conductivity(i - 1, j, k));
        double divx = (kx_p * (Tx_p - T0) - kx_m * (T0 - Tx_m)) / (dx * dx);


        double Ty_p = grid.T(i, j + 1, k); double ky_p = k_face(k0, grid.conductivity(i, j + 1, k));
        double Ty_m = grid.T(i, j - 1, k); double ky_m = k_face(k0, grid.conductivity(i, j - 1, k));
        double divy = (ky_p * (Ty_p - T0) - ky_m * (T0 - Ty_m)) / (dy * dy);

        double Tz_p = grid.T(i, j, k + 1); double kz_p = k_face(k0, grid.conductivity(i, j, k + 1));
        double Tz_m = grid.T(i, j, k - 1); double kz_m = k_face(k0, grid.conductivity(i, j, k - 1));
  ...
  // Нижняя грань z=nz-1 (адиабат всегда — образец большой по сравнению с прогретой зоной)
  for (int j = 0; j < ny; ++j)
    for (int i = 0; i < nx; ++i)
      apply_lateral(i, j, nz - 1, i, j, nz - 2);

  // ----- Верхняя грань z=0: либо адиабат, либо лазер -----
  // Идея: для приграничного узла k=0 поток через нижнюю грань (между k=0 и k=1)
  ...
  for (int j = 1; j < ny - 1; ++j) {
    for (int i = 1; i < nx - 1; ++i) {
      ...
      // Поток вниз (между k=0 и k=1)
      double Tz_p = grid.T(i, j, k + 1); double kz_p = k_face(k0, grid.conductivity(i, j, k + 1));
      double flux_down = -kz_p * (Tz_p - T0) / dz;  // F_{1/2} в форме -k dT/dz
